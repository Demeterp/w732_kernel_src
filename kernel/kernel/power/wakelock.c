...
static void expire_wake_locks(unsigned long data)
{
    ...
    if (has_lock == 0) {
        pr_info("[expire_wake_locks]: suspend_work_queue suspend_work\n");
...
void wake_lock_init(struct wake_lock *lock, int type, const char *name)
{
    ...
    if (unlikely(lock->flags & WAKE_LOCK_INITIALIZED)) {
        pr_err("[wake_lock_init] wakelock %s init already(0x%x)\n", lock->name, lock->flags);
        BUG();
    }
...
{
    if (wakelock_debug_mask & DEBUG_WAKE_LOCK)
        pr_info("[wake_lock_timeout] wakeup wake lock: %s timeout: %ld\n", lock->name, timeout);
...
void wake_unlock(struct wake_lock *lock)
{
    ...
    if (type == WAKE_LOCK_SUSPEND) {
        ...
        if (wakelock_debug_mask & DEBUG_WAKE_LOCK)
            pr_info("[wake_unlock]: has_lock = %ld\n" , has_lock);        
        if (has_lock > 0) {
        ...
        if (lock == &main_wake_lock) {
            int dbg_mask_bakup = wakelock_debug_mask;
            wakelock_debug_mask |= DEBUG_SUSPEND;
            if (wakelock_debug_mask & DEBUG_SUSPEND)
                print_active_locks(WAKE_LOCK_SUSPEND);
            wakelock_debug_mask = dbg_mask_bakup;
#ifdef CONFIG_WAKELOCK_STAT
            update_sleep_wait_stats_locked(0);
#endif
        }
