...
#define charger_UNDER_VOL                 2   /*yanfang 20120307 modify for charger check*/
#define BATTERY_OVER_TEMP		      3   /*yanfang 20120307 modify for battery temprature check*/
#define BATTERY_UNDER_TEMP             4   /*yanfang 20120307 modify for battery temprature check*/
...
typedef struct 
{
    ...
    UINT32      ADC_I_SENSE;
    UINT32      bat_temperature_status;  /*yanfang 20120307 modify*/
} PMU_ChargerStruct;
...
#endif
static int charger_low_counter=0; //yanfang
    ...
    if (upmu_is_chr_det() == KAL_TRUE)
    {
        ...
        if ( BMT_status.charger_vol >= V_CHARGER_MAX )
        {
			...
			else
			{
				...
				{
					//printf("[BATTERY]Charger under ovp voltage!!\r\n");                    
					BMT_status.charger_protect_status = 0;
					BMT_status.bat_charging_state = CHR_PRE;
				}
			}
...
PMU_STATUS BAT_ConstantCurrentModeAction(void)
{
    ...
    if ( BMT_status.bat_vol > V_CC2TOPOFF_THRES )
    {
		...
		if(g_temp_CC_value == AC_CHARGER_CURRENT)
		{
			...
			batteryCurrentSum = AC_CHARGER_CURRENT * BATTERY_AVERAGE_SIZE;//LK@I3000,650->650
		}
	#if (V_CHARGER_COMPATIBLE_ENABLE == 1)
            	...
            	|| mtk_wdt_boot_check()==WDT_BY_PASS_PWK_REBOOT || g_boot_arg->boot_reason==BR_TOOL_BY_PASS_PWK )			
            {
                ...
                {	
                    if (low_bat_boot_display == 0)
                    {
						...
						/*punk add for c8 bug629,  fill the screen with a whole black image */
						mt65xx_disp_fill_rect(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT, 0x0);
						mt65xx_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
						mt65xx_disp_wait_idle();
