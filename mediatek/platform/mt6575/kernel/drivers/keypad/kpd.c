...
#if  defined(I5000_A10)//mr or hall  sensor   amy0601 
//#define KPD_DRV_CTRL_MHALL 1 
#define KPD_MHALL_EINT  		CUST_EINT_MHALL_NUM
#define KPD_MHALL_DEBOUNCE  	CUST_EINT_MHALL_DEBOUNCE_CN		/* ms */
#define KPD_MHALL_POLARITY	    CUST_EINT_MHALL_POLARITY
#define KPD_MHALL_SENSITIVE  	CUST_EINT_MHALL_SENSITIVE
#define KPD_MHALL_SENSITIVE_EN	CUST_EINT_MHALL_DEBOUNCE_EN
#endif
...
/* for MR */
#if KPD_DRV_CTRL_MHALL
static void kpd_mhall_handler(unsigned long data);
static DECLARE_TASKLET(kpd_mhall_tasklet, kpd_mhall_handler, 0);
static u8 kpd_mhall_state = !KPD_MHALL_POLARITY;
#endif
...
{
	bool mhall;
	u8 old_state = kpd_mhall_state;

	kpd_mhall_state = !kpd_mhall_state;
	mhall = (kpd_mhall_state == !!KPD_MHALL_POLARITY);
	/* for mhall, 0: mhall opened, 1: mhall closed */
	kpd_print("report mhall = %s\n", mhall ? "closed" : "opened");
	
    kpd_print("report mhall = %d,kpd_suspend=%d\n", mhall, kpd_suspend);

    if (mhall) // mhall closed
    {
        if (!kpd_suspend) 
        {
            kpd_print(" mhall closed  report powerkey\n");
	...
	else //mhall opened
	{
	    if (kpd_suspend) 
        {
            kpd_print(" mhall opened  report powerkey\n");
            input_report_key(kpd_input_dev, KPD_PWRKEY_MAP, 1);       
            input_report_key(kpd_input_dev, KPD_PWRKEY_MAP, 0);
		}
	...
	mt65xx_eint_set_polarity(KPD_MHALL_EINT, old_state);
	mt65xx_eint_unmask(KPD_MHALL_EINT);
}

static void kpd_mhall_eint_handler(void)
{
	tasklet_schedule(&kpd_mhall_tasklet);
}
#endif
...
static void kpd_lowpower_handler(unsigned long data)
{
	...
	/* for SW_LID, 1: lid open => slid, 0: lid shut => closed */
	//input_report_switch(kpd_input_dev, SW_LID, lowpower);
	//input_sync(kpd_input_dev);
	kpd_print("report lowpower QWERTY ,state=%s\n", lowpower ? "on" : "off");
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	mt65xx_eint_set_sens(KPD_MHALL_EINT, KPD_MHALL_SENSITIVE);
	mt65xx_eint_set_hw_debounce(KPD_MHALL_EINT, KPD_MHALL_DEBOUNCE);
	mt65xx_eint_registration(KPD_MHALL_EINT, true, KPD_MHALL_POLARITY,
	                         kpd_mhall_eint_handler, false);
#endif
