...
//extern void mt6575_irq_mask(unsigned int line);
//extern void mt6575_irq_unmask(unsigned int line);
...
{ 
	...
	if(tpd_sample_mode&FAV_MODE_HW || tpd_sample_mode&RAW_DATA_MODE) {	
		if(__raw_readw(AUXADC_TP_CON1)&(1<<4))
		{
			...
			//mt6575_irq_unmask(MT65XX_TOUCH_BATCH_LINE);
			//TPD_DMESG("Invalid flag\n");
			//return IRQ_HANDLED;
		}
		tpd_fav_switch(0);
