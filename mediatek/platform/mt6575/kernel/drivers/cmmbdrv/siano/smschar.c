...
static int smschar_ioctl(struct file *file,
			 unsigned int cmd, unsigned long arg)
{
	struct smschar_device_t *dev = file->private_data;
...
{
#ifndef mtk_fp_spi
       printk("[Siano_CharDriver]SMSDev_probe start\r\n");
...
static int SMSDev_suspend(struct spi_device *spi)
{
...
#ifdef MTK_SPI
#ifndef mtk_fp_spi                     //xingyu add   //mtk_fp_spi delete
