...
#if defined(SUPPORT_CALLINGLIGHT) //chenhaojun
...
static enum hrtimer_restart calling_light_decate(struct hrtimer *timer)
{
	...
	ACCDET_DEBUG("[Accdet] current_status = %d",current_status1);

	current_status2 = INREG32(ACCDET_MEMORIZED_IN) & 0x3; //A=bit1; B=bit0
	ACCDET_DEBUG("[Accdet] current_status = %d",current_status2);
...
void accdet_eint_work_callback(struct work_struct *work)
{
	...
	else
	{
		...
		hrtimer_start(&accdet_timer, ktime_set(0, 1000000000), HRTIMER_MODE_REL); //check again after 1000ms
		ACCDET_DEBUG("[Accdet] EINT PLUG IN  \n");
	}
}
#else
void accdet_eint_work_callback(struct work_struct *work)
...
//longxuewei add      
	    if( ACC_HEADSET_FLAG== accdet_key_event)
            {
                ACCDET_DEBUG("[Accdet] HEADSET TEST BUTTON !\n");
                input_report_key(kpd_accdet_dev, KEY_F24, 1);
                input_report_key(kpd_accdet_dev, KEY_F24, 0);
                input_sync(kpd_accdet_dev);
            }
//longxuewei end
        }
...
//longxuewei add
		...
		{
		  ACCDET_DEBUG("[accfix] key DOWN\n");
		  input_report_key(kpd_accdet_dev, 226 , 1); //KEY_H   KEY_VOLUMEUP
		  isDown = true;
		  if(is_long_press()) //was working
		  {
			input_sync(kpd_accdet_dev);
		  } else {
			ACCDET_DEBUG("[accfix] key UP\n");
			input_report_key(kpd_accdet_dev, 226 , 0);
			isDown = false;
			input_sync(kpd_accdet_dev);
		  }  
		}
...
void accdet_work_callback(struct work_struct *work)
{
	...
	//enable_irq(MT6575_ACCDET_IRQ_ID);
}
...
//longxuewei add
	__set_bit(KEY_F24, kpd_accdet_dev->keybit);
	__set_bit(226, kpd_accdet_dev->keybit);
//longxuewei add end    
