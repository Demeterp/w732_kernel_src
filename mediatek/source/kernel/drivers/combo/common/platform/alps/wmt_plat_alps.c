    ...
    )
{
    ...
    {
    case PIN_STA_INIT:
        #if defined(SIMCOM_I3000)//LK@I3000,to compatible two hw version
        /*set to gpio output low, disable pull*/
        mt_set_gpio_pull_enable(GPIO12, GPIO_PULL_DISABLE);
        mt_set_gpio_dir(GPIO12, GPIO_DIR_OUT);
        mt_set_gpio_mode(GPIO12, GPIO_MODE_GPIO);
        mt_set_gpio_out(GPIO12, GPIO_OUT_ZERO);

         /*set to gpio output low, disable pull*/
        mt_set_gpio_pull_enable(GPIO126, GPIO_PULL_DISABLE);
        mt_set_gpio_dir(GPIO126, GPIO_DIR_OUT);
        mt_set_gpio_mode(GPIO126, GPIO_MODE_GPIO);
    ...
    case PIN_STA_DEINIT:
        #if defined(SIMCOM_I3000)//LK@I3000,to compatible two hw version
        /*set to gpio input low, pull down enable*/
        mt_set_gpio_mode(GPIO12, GPIO_COMBO_PMU_EN_PIN_M_GPIO);
        mt_set_gpio_dir(GPIO12, GPIO_DIR_IN);
        mt_set_gpio_pull_select(GPIO12, GPIO_PULL_DOWN);
        mt_set_gpio_pull_enable(GPIO12, GPIO_PULL_ENABLE);
        
        /*set to gpio input low, pull down enable*/
        mt_set_gpio_mode(GPIO126, GPIO_COMBO_PMU_EN_PIN_M_GPIO);
        mt_set_gpio_dir(GPIO126, GPIO_DIR_IN);
