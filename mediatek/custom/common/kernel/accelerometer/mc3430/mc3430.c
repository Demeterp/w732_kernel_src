...
#define MC3210_DATA_LEN       3// 6  //yanfang 20120521
...
static int MC3210_SetDataResolution(struct mc3210_i2c_data *obj)
{
	...
	//reso = (dat & MC3210_RANGE_MASK)>>2;  //yanfang 20120521
...
static int MC3210_ReadData(struct i2c_client *client, s16 data[MC3210_AXES_NUM])
{
	...
	#else
	else if((err = hwmsen_read_block(client, addr, buf, 0x03)))  // yanfang 20120521
	#endif
	{
		GSE_ERR("error: %d\n", err);
	}
