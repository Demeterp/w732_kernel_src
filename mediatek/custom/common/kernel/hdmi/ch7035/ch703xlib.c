...
// MTK-Add-Start
//#define os_shift_udiv(divisor, dividend, shift)	\
//		(uint32)(((uint64)(dividend) << (shift)) / (divisor))
		
//#define os_shift_udiv_roundup(divisor, dividend, shift)	\
//		(uint32)((((uint64)(dividend) << (shift)) + ((divisor) -1)) / (divisor))
...
// MTK-Add-Start
uint32 os_shift_udiv(divisor, dividend, shift)
{
    uint64 result, n, mod;
...
uint32 os_shift_udiv_roundup(divisor, dividend, shift)
{
    uint64 result, n, mod;
...
#define MCU_REG_GROUPS			5
static uint8 es_map[MCU_REG_GROUPS][2] = {
	{ 0x26, 2 },	//	0x26, 0x27
	{ 0x42, 6 },	//	0x42, 0x43, 0x44, 0x45, 0x46, 0x47
	{ 0x6A, 1 },	//	0x6A
	{ 0x51, 3 },	//	0x51, 0x52, 0x53
	{ 0x57, 4 },	//	0x57, 0x58, 0x59, 0x5A
};
...
#else
#define	MAX_REQUEST_RETRIES		20
#define EACH_REQUEST_SLEEP		100
#endif
		...
		uint8 *buf, int num)
{
	...
	for(j = MAX_REQUEST_RETRIES; j != 0; j--) {
		...
		if ((reqCmd & 0x3F) != (0x7F & i2c_read_byte(addr, 0x4F)))
			continue;	// wait
		...
		// read responce data
		if (num > 0) {
			if ((0xFF != reqIdx) && (0x80 & i2c_read_byte(addr, 0x50))) {
				printk("fw_McuRequest MCU responce error %d\n", j); // MTK-Modify
				return ERR_MCU_RESPONCE;
			}

			i2c_write_page(addr, MCU_DATA_PAGE);
			for(i=0; (i < MCU_REG_GROUPS) && (num > 0); i++) {
				l = es_map[i][1];
				if (l > num) l = num;
				i2c_read_block(addr, es_map[i][0], l,  buf);
				buf += l;
				num -= l;
			}
		}
...
#define fw_getEdid16Bytes(addr, offset, buf)		\
		fw_McuRequest(addr, offset, 0x41, buf, 16);
...
static int32 fw_getEdidBlock(uint8 addr, uint8 * buf, uint8 block)
{
	...
	for(i = block + 8; block != i; block++, buf += 16) {
		ret = fw_getEdid16Bytes(addr, block, buf);
		if (ERR_NO_ERROR != ret) {
...
//	uint32 vao_t;	-WYC
	...
	iicbuf_write_ex(PAC8_1_ENABLE, pac8_1_enable_reg);
...
//	}
	...
	for(blk = 1; blk <= edid[126]; blk++) {
		ret = fw_getEdidBlock(addr, &edid[blk*128], (blk << 3) | whichEDID);
		if (ERR_NO_ERROR != ret)
			break;
