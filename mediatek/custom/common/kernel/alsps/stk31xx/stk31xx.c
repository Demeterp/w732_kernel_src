...
static int stk31xx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	atomic_set(&obj->als_suspend, 0);
	atomic_set(&obj->als_cmd_val, stk31xx_als_cmd_val );//obj->hw->als_cmd_val
	atomic_set(&obj->ps_cmd_val, stk31xx_ps_cmd_val);//obj->hw->ps_cmd_val 
	gain_setting = stk31xx_ps_gain_setting; //obj->hw->ps_gain_setting;
	...
	if(obj->hw->polling_mode_ps == 0)
	{
	  atomic_set(&obj->ps_cmd_val,  stk31xx_ps_cmd_val | 0x02); //(obj->hw->ps_cmd_val)
	  APS_LOG("enable PS interrupt\n");
	}
	if(obj->hw->polling_mode_als == 0)
	{
	  atomic_set(&obj->als_cmd_val,  stk31xx_als_cmd_val | 0x02); //(obj->hw->als_cmd_val)
